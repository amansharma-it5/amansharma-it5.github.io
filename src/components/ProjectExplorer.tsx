...
export default function ProjectExplorer({ compact = false, initialFilter = 'all' }: { compact?: boolean; initialFilter?: string }) {
  ...
  const visibleProjects = useMemo(() => projects.filter((project) => {
    ...
    const haystack = `${project.name} ${project.eyebrow} ${project.category} ${project.status} ${project.summary} ${project.stack.join(' ')}`.toLowerCase();
  ...
  return (
    <div className="explorer" data-project-explorer data-hydrated={hydrated}>
