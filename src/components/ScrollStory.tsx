...
export default function ScrollStory({ project, beats }: { project: Project; beats?: ProjectStoryBeat[] }) {
  ...
  return (
    <div ref={root} className="scroll-story" style={{ '--story-count': storyBeats.length, '--story-accent': project.accent } as CSSProperties}>
      <div className="scroll-story__media">
        <div className="scroll-story__sticky">
          <ProjectVisual project={project} large />
          <div className="scroll-story__focus" aria-hidden="true">
            <span>{currentBeat.label} / {String(active + 1).padStart(2, '0')}</span>
            <strong>{currentBeat.title}</strong>
          </div>
          <span className="scroll-story__count">{String(active + 1).padStart(2, '0')} / {String(storyBeats.length).padStart(2, '0')}</span>
        </div>
      </div>
